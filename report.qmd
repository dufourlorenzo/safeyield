...
**Scope and scalability.** The current prototype is scoped to mainland Portugal between 1980 and 2021. At this scale the dataset is manageable on a single machine, but the Portugal scope is a deliberate starting point, not a ceiling. Extending the same pipeline to a continental or global footprint changes the nature of the problem entirely. In the absence of clean global tabular data, such an expansion would rely on raw satellite imagery to map fire scars. At a 10-meter per pixel resolution, a single 1km x 1km tile requires approximately 30KB of storage. Scaling this across Earth's 148.9 million km² of landmass means a single global snapshot exceeds 4.4 terabytes. Historical analysis demands continuous snapshots over decades—easily compounding into hundreds of terabytes or petabyte-scale data. Combining these massive raster layers with heterogeneous real estate records from dozens of national markets, each with its own schema, language, and update frequency, produces a dataset that is genuinely large, heterogeneous, and continuously refreshed. Processing and joining those sources in near real-time is a distributed computing problem that local tools cannot handle. The Portugal prototype demonstrates that the modeling logic works; the architecture is designed to scale out to this wider, petabyte-scale scope.
