...
**Scope and scalability.** The current prototype is scoped to mainland Portugal between 1980 and 2021. At this scale the dataset is manageable on a single machine, but the Portugal scope is a deliberate starting point, not a ceiling. Extending the same pipeline to a continental or global footprint changes the nature of the problem entirely. Wildfire records from agencies such as NASA FIRMS, the European Forest Fire Information System (EFFIS), or national registries in the US, Australia, Canada, or Greece arrive in different formats: satellite raster layers, government CSV exports, real-time GeoJSON feeds, and remote-sensing products at varying resolutions. Combining those with real estate data from dozens of national markets, each with its own schema, language, and update frequency, produces a dataset that is genuinely large, heterogeneous, and continuously refreshed. Processing and joining those sources in near real-time is a distributed computing problem that local tools cannot handle. The Portugal prototype demonstrates that the modeling logic works; the architecture is designed to scale out to that wider scope.
...
**Accessibility.** The SafeYield dashboard is built in Python and hosted on Streamlit Community Cloud for free public access. It provides 3D mapping and filtering capabilities without requiring local compute. Source code, raw data, and the base `wildfire.ipynb` notebook remain available in the public [Github repository](https://github.com/dufourlorenzo/safeyield).
